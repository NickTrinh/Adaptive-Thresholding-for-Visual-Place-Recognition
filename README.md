...
### Descriptor-agnostic validation (EigenPlaces, same protocol)

We re-ran the entire pipeline with EigenPlaces (ResNet50, 2048-d) to test
whether the rejection-dominance pattern is a property of the thresholding
mechanism or of the descriptor.
...
The pattern transfers: per-place thresholds outperform per-query
thresholds (Vys-thresh) on **rejection rate on all six datasets** under
EigenPlaces just as under DINOv2 SALAD, by factors of ~1.5× to >75×.
Absolute F1 numbers are lower across the board because EigenPlaces is a
materially weaker descriptor (the closed-set baseline drops by 5 points
on Nordland and over 30 on Freiburg — every method inherits that
ceiling), but the *relative* ordering between thresholding mechanisms
is preserved. This supports the §III-A claim that per-place thresholding
is a descriptor-agnostic improvement on the thresholding component.

Generated by `experiments/final_all_datasets_eigenplaces.py`; descriptors
extracted by `experiments/extract_eigenplaces_all.py`.
...
| `experiments/extract_dinov2_salad_all.py` | DINOv2 SALAD descriptor extraction for all 6 datasets via `torch.hub.load("serizba/salad", "dinov2_salad")`; CPU fallback. |
| `experiments/extract_eigenplaces_all.py` | EigenPlaces (ResNet50, 2048-d) descriptor extraction for the same 6 datasets — used for the descriptor-agnostic validation tables. Wipes target caches first to guarantee a fresh run. |
| `experiments/final_all_datasets_eigenplaces.py` | Same pipeline as `final_all_datasets.py` but reads EigenPlaces caches. Produces Tables IV and V in the paper. |
| `experiments/robustness_sweep.py` | Sweeps the four data-touching constants (α, m, h, divisor) on the natural open-set protocol across all six datasets. Confirms α/m flat; surfaces the divisor F1-vs-rejection Pareto tradeoff. Not in the paper. |
| `experiments/bayes_k_comparison.py` | Compares `k = sep/2` to the per-place Bayes-derived `k = sep / (1 + σ⁺/σ⁻)`. Documented internally; not in the paper because Bayes trades rejection for F1. |
...
(Tables II and III, DINOv2 SALAD) and `experiments/final_all_datasets_eigenplaces.py`
(Tables IV and V, EigenPlaces); the JSON files they write are the source
of truth for the result tables.
